...
export async function GET(request: Request): Promise<Response> {
  ...
  const staffDtos: StaffDto[] = await Promise.resolve(
...
export async function POST(request: Request): Promise<Response> {
  ...
  const roomNoSet = new Set(
    rows.map((r) => r.roomNo).filter((v): v is string => !!v)
  );
  const deptNameSet = new Set(
    rows.map((r) => r.departmentName).filter((v): v is string => !!v)
  );

  const rooms =
    roomNoSet.size > 0
      ? await prisma.room.findMany({
          where: { roomNo: { in: Array.from(roomNoSet) } },
        })
      : [];
  ...
  const data = rows.map((r) => ({
    ...
    departmentId: r.departmentName
      ? deptNameToId.get(r.departmentName) ?? null
      : null,
  }));
